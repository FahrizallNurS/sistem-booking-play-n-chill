{{-- Modal khusus F&B (data pesanan masih placeholder). Menerima $t dari @include(..., ['t' => $t]) --}}
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <span class="text-dark">{{ $t->id_pos ?? '-' }}</span>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-0">
                            ...
                            <span class="text-dark">{{ \Carbon\Carbon::parse($t->waktu_mulai)->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <span style="color: #e83e8c;">{{ $t->kode_booking ?? '-' }}</span>
                        </div>
                        <div class="mb-3">
                            <small class="text-muted d-block mb-1">Ruangan</small>
                            <span class="text-dark">{{ $t->ruangan ?? '-' }}</span>
                        </div>
                        <div class="mb-0">
                            <small class="text-muted d-block mb-1">Status Pesanan</small>
                            {{-- placeholder sementara (belum pakai x-status-badge, menunggu data asli) --}}
                            <span class="badge" style="background-color: #2ecc71; color: white; padding: 6px 16px; border-radius: 4px; font-weight: normal;">
                                {{ ucfirst($t->status_sewa ?? 'Selesai') }}
                            </span>
                        </div>
                    </div>
                </div>
                            ...
                            @if(isset($t->items))
                                @foreach($t->items as $idx => $item)
                                    <tr style="border-bottom: 1px solid #e2e8f0;">
                                        ...
                                        <td class="py-2 text-muted">{{ $item->produk }}</td>
                                        <td class="py-2 text-muted">Rp {{ number_format($item->harga, 0, ',', '.') }}</td>
                ...
                <div class="d-flex justify-content-end mb-4">
                    <table class="table-borderless table-sm text-right" style="width: 250px;">
                        <tr>
                            ...
                            <td class="text-dark pb-1">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td class="font-weight-bold text-dark pt-1" style="font-size: 1.1rem;">Total:</td>
                            <td class="font-weight-bold pt-1" style="color: #6f42c1; font-size: 1.2rem;">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</td>
                        </tr>
                    </table>
                </div>
